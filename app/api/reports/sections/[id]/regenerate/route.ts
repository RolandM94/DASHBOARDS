...
export async function POST(
  ...
  const { job, result } = await runWithJob(supabase, projectId, "generate_section", 1, async (updateProgress) => {
    await updateProgress("Regenerating section", 0, 1);
    ...
    await logReportAction(supabase, user.id, "regenerate_section", {
    ...
    await updateProgress("Section regenerated", 1, 1);
    return output;
  });

  if (job.status === "failed") {
    const message = job.errorMessage ?? "Section regeneration failed";

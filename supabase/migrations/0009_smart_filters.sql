-- ─────────────────────────────────────────────────────────────────────────────
-- Migration 0009 — Dataset-native smart analytical filter conditions
-- Extends aggregate_dataset with a p_smart_filter_conditions parameter that
-- accepts SQL WHERE fragments resolved from the current dataset's own fields.
-- Also updates the grant statement with the new parameter signature.
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  ...
  -- Each entry is a SQL WHERE fragment resolved from the active dataset's
  -- field metadata. The API only sends fragments generated by the app's
  -- dataset-native smart filter resolver.
...
-- Re-grant with updated parameter signature (7 params now instead of 6)
